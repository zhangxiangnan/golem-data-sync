...
## 2026-09-24 · SeaTunnel 实验台

- `/lab`、参数目录、本机集群、实验配置、运行详情与 2～4 次运行对比上线，原单表任务及浏览保持独立。
- JSON 为唯一配置源；未知字段保留，凭据引用/加密秘密、不可变加密执行快照、外部 jobId、事件及 2 秒指标采样已接入。Flyway V3 独立实验存储，无新增依赖。
- 本机目录 57 组、376 项条目，来自 Options/Factory/嵌套配置定义，20 个 Transform 可查；重新生成后与资源逐字一致。定义默认值和实际引擎返回明确区分。
- 真实 Zeta 验收：Transform 120→80，多表和多分支 20/10/120；普通 JDBC 与 XA 保存点恢复均完成 100000 条，逐字段差异 0；运行中 API 重启按原 jobId 完成，取消独立标识。
- 结构变化完成新增/删除/改类型与保存策略成功失败记录；性能完成并行度 1/2/4、batch_size 和 split.size 比较，以及 query 的 partition_num=4 读取。
- 后端 53 项测试、前端 28 项测试、类型检查和生产构建通过；真实浏览器验收编辑往返、提交、运行、对比、目录和旧浏览入口。
- 原演示库仍为源 20/10/120、目标 0/0/120，原任务未修改。上面的“尚未完成真实链路验收”是第一阶段历史状态；本次独立实验台的真实测试见下方文档。
- 经用户明确授权，专用实验账号获得全局 XA_RECOVER_ADMIN，XA 恢复实测成功；没有据此宣称所有场景 exactly-once。

使用与边界：[SEATUNNEL-LAB.md](SEATUNNEL-LAB.md)。运行 ID、结果与复现实验：[LAB-EXPERIMENTS.md](LAB-EXPERIMENTS.md)。
