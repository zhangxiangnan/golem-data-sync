...
export function Chart({series,label}:{series:{name:string;values:(number|null)[];times:string[]}[];label:string}){
 ...
 return <div className="lab-chart"><h4>{label}</h4><svg viewBox={`0 0 ${width+60} ${height+40}`} role="img" aria-label={label}><text x="0" y="14" fontSize="10" fill="#667085">{max.toFixed(1)}</text><line x1="50" y1={height} x2={width+50} y2={height} stroke="#e5e9f2"/>{series.map((s,j)=>{const start=Date.parse(s.times[0]??""),end=Date.parse(s.times.at(-1)??"");const coords=s.values.map((v,i)=>v===null?null:`${50+(end>start?(Date.parse(s.times[i])-start)/(end-start):0)*width},${height-((v??0)/max)*(height-16)}`);return <g key={s.name}>{coords.map((p,i)=>p?<circle key={i} cx={p.split(",")[0]} cy={p.split(",")[1]} r="2" fill={colors[j%4]}><title>{s.times[i]}：{s.values[i]}</title></circle>:null)}{coords.map((p,i)=>i>0&&p&&coords[i-1]?<line key={i} x1={coords[i-1]!.split(",")[0]} y1={coords[i-1]!.split(",")[1]} x2={p.split(",")[0]} y2={p.split(",")[1]} stroke={colors[j%4]}/>:null)}</g>})}<text x="50" y={height+25} fontSize="10" fill="#667085">各运行从首次采样开始 · 横轴为该运行已采样时段</text></svg><div className="lab-links">{series.map((s,i)=><span key={s.name} style={{color:colors[i%4]}}>{s.name} · {s.values.length} 次采样</span>)}</div></div>;
}
