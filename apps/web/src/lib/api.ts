import type { ColumnInfo, DashboardSummary, DataSource, DataSourceInput, JobValidation, SyncJob, SyncJobInput, SyncRun, SystemStatus, TableInfo } from "./types";
...
export const api = {
  ...
  configPreview: (id: string) => request<{ format: string; content: string }>(`/api/sync-jobs/${id}/config-preview`),
  startRun: (id: string) => request<SyncRun>(`/api/sync-jobs/${id}/runs`, { method: "POST" }),
