...
export default function RunDetailPage() {
  ...
  return <>
    ...
    {run.stale && <div className="alert warning" style={{ marginBottom: 18 }}><strong>指标可能已过期</strong><span>暂时无法连接 SeaTunnel：{run.lastPollError}</span></div>}
    {run.errorMessage && <div className="alert error" style={{ marginBottom: 18 }}><strong>执行错误</strong><span>{run.errorMessage}</span></div>}
    <section className="metrics-grid" style={{ gridTemplateColumns: "repeat(4, minmax(0,1fr))" }}><MetricCard icon={DatabaseZap} label="源端读取" value={formatNumber(run.sourceReadCount)} hint={`${formatNumber(run.sourceBytes)} bytes`} /><MetricCard icon={UploadCloud} label="目标写入" value={formatNumber(run.sinkWriteCount)} hint={`${formatNumber(run.sinkBytes)} bytes`} tone="green" /><MetricCard icon={Gauge} label="当前吞吐" value={`${run.sinkQps.toFixed(1)} r/s`} hint={`读取 ${run.sourceQps.toFixed(1)} r/s`} tone="amber" /><MetricCard icon={Timer} label="运行耗时" value={duration(run.startedAt ?? run.createdAt, run.finishedAt)} hint={active ? "持续更新" : "最终耗时"} /></section>
    <div className="detail-grid"><section className="panel"><div className="panel-header"><div><h2>状态时间线</h2><p>仅记录真实状态变化，不生成伪进度。</p></div></div><div className="panel-body"><div className="timeline">{run.events.map((event) => <div className="timeline-item" key={event.id}><strong><StatusBadge status={event.status} /></strong><p>{event.message}</p><time>{formatDate(event.createdAt)}</time></div>)}</div></div></section><aside className="panel"><div className="panel-header"><div><h2>运行信息</h2><p>平台与引擎关联信息</p></div></div><div className="panel-body"><div className="summary-line"><span>SeaTunnel jobId</span><strong>{run.seatunnelJobId ?? "等待分配"}</strong></div><div className="summary-line"><span>创建时间</span><strong>{formatDate(run.createdAt)}</strong></div><div className="summary-line"><span>开始时间</span><strong>{formatDate(run.startedAt)}</strong></div><div className="summary-line"><span>结束时间</span><strong>{formatDate(run.finishedAt)}</strong></div><div className="summary-line"><span>最后更新</span><strong>{formatDate(run.updatedAt)}</strong></div></div></aside></div>
  </>;
}
